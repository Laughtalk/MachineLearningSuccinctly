...
namespace RBFNetwork
{
    class RadialNetwork
    {
        ...
        /// <summary>
        /// 为每个隐藏节点计算一个中心点
        /// 这里是随机选取numHidden个点，多次尝试，寻找距离总和最大的组合
        /// 但是这样感觉很不科学，感觉还是应该先用分类算法先分类
        /// </summary>
        ...
        /// <summary>
        /// 返回两个向量对应项差的绝对值的平均数（Manhattan Distance）
        /// </summary>
        ...
        private double[] DoWeights(double[][] trainData, int maxIterations)
        {
            ...
            while (iteration < maxIterations)
            {
                ...
                Shuffle(ref sequence); // move particles in random sequence
        ...
        private static void Shuffle(ref int[] sequence)
        {
