...
namespace NaiveBayes
{
    class BayesClassifier
    {
        ...
        /// 实际上只是对训练数据进行计数
        /// </summary>
        /// <param name="trainData">用于训练的数据</param>
        ...
        /// 计算给定x的情况下，预测为y的概率
        /// </summary>
        /// <param name="yValue">待判断预测值</param>
        /// <param name="xValues">用于预测的数据</param>
        /// <returns>返回yValue的概率</returns>
        public double Probability(string yValue, string[] xValues)
        {
            ...
            for (int k = 0; k < 2; ++k) // each y-value
            {
                for (int i = 0; i < numFeatures; ++i)
                {
                    ...
                    int valIndex = x[i];
                    int depIndex = k;
                    conditionals[k][i] = (double)(jointCounts[attIndex][valIndex][depIndex]) / (double)(dependentCounts[depIndex]);
                }
            }
            ...
            for (int k = 0; k < 2; ++k)
                unconditionals[k] = (double)(dependentCounts[k]) / (double)(totalDependent);
            // compute partials
