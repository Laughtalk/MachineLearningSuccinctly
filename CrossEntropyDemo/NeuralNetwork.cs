...
namespace CrossEntropyDemo
{
    class NeuralNetwork
    {
        ...
        private double[] ComputeOutputs(double[] xValues)
        {
            ...
            for (int j = 0; j < numOutput; ++j)   // compute h sum of weights * hOutputs
                for (int i = 0; i < numHidden; ++i)
        ...
        private void UpdateWeights(double[] tValues, double learnRate)
        {
            ...
            for (int i = 0; i < oGrads.Length; ++i)
            {
                ...
                // MCEE
                oGrads[i] = (tValues[i] - outputs[i]); // cross-entropy version drops (1-y)(y) term!
            }
        ...
        {
            // train a back-prop style NN classifier
        ...
        private double MeanSquaredError(double[][] trainData) // used as a training stopping condition
        {
            ...
            for (int i = 0; i < trainData.Length; ++i) // looks like (6.9 3.2 5.7 2.3) (0 0 1)  (no parens)
            {
                ...
                double[] yValues = this.ComputeOutputs(xValues); // using current weights
                for (int j = 0; j < numOutput; ++j)
                {
